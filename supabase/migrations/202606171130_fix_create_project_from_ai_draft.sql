...
begin
  ...
  for i in 0 .. jsonb_array_length(v_work_packages) - 1 loop
    ...
    insert into public.work_packages (
      ...
      v_estimate_id,
      v_work_package ->> 'name',
      (v_work_package ->> 'estimated_hours')::numeric,
      i,
      true
  ...
  for i in 0 .. jsonb_array_length(v_tasks) - 1 loop
    ...
    v_work_package_id := (
      v_work_package_by_name_idx ->> (v_task_json ->> 'work_package_name')
    )::uuid;
  ...
  return jsonb_build_object(
    ...
    'work_package_count', jsonb_array_length(v_work_packages)
  );
