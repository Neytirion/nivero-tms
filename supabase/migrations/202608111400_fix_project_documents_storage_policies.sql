-- Ensure project-documents bucket is private and has correct RLS policies.

update storage.buckets
set public = false
where id = 'project-documents';

-- Helper: extract project UUID from storage object path ({projectId}/filename)
...
-- Helper: check that the authenticated user is a member of the project
...
begin
  if p_user_id is null or v_project_id is null then return false; end if;
  return exists (
    ...
    where p.id = v_project_id
      and (
        ...
        or exists (
          ...
          where pm.project_id = v_project_id and pm.user_id = p_user_id
        )
      )
  );
end;
...
-- Drop all existing project-documents storage policies before recreating
...
drop policy if exists "Project docs read by project members" on storage.objects;
drop policy if exists "Project docs upload by project members" on storage.objects;
drop policy if exists "Project docs update by project members" on storage.objects;
drop policy if exists "Project docs delete by project members" on storage.objects;
