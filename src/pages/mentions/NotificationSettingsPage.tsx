...
const STORAGE_KEY = 'nivero-notification-settings-v1'
...
export function NotificationSettingsPage() {
  ...
  const toggleItems: Array<{ key: keyof NotificationSettings; title: string; description: string }> = [
    {
      key: 'slackEnabled',
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="mb-5 flex items-center justify-between gap-3">
          ...
          <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${isSaved ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
            {isSaved ? 'Saved' : 'Autosave'}
          </span>
        </div>
